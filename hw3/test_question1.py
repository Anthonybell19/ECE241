...
class HashTable:
    ...
    def put(self, key, data):
        ...
        if len(self.slots) > 0:
            for i in range(0, len(self.slots) - 1):
                if self.slots[i] is not None and self.slots[i].search(key) is not None:
                    self.slots[i].head.setData(data)
            if self.slots[hashValue] is None:
                self.slots[hashValue] = tempList
            if self.slots[hashValue] is not None and self.slots[hashValue].search(key) is None:
